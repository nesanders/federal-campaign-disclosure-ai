...
def main() -> None:
    ...
    meta = {
        ...
        "methodology_notes": [
            "Vendor matches are text matches against payee name, disbursement purpose, category description, and memo text. See config/vendors.yaml for the full pattern list and its provenance.",
            ...
            "Google's Gemini is frequently bundled into a Google Workspace subscription a campaign already pays for email and documents, so it often has no separate line item the way a standalone ChatGPT or Claude subscription does. This likely understates Google's real usage more than it does OpenAI's or Anthropic's -- a limitation of disbursement-based analysis.",
            "'% of total spend' denominators are each committee's total reported operating expenditure (Schedule B), excluding FEC memo entries to avoid double-counting a lump-sum payment and its own itemized breakdown. For party/incumbency/chamber/age/time-series charts, the denominator is the combined total spend of the House/Senate candidates in that slice who have at least one AI-vendor disbursement, so these percentages answer 'how big is AI spend relative to everything else these AI-using campaigns spend.'",
